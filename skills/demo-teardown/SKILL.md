...
Read `{workspace}/{slug}/.env`. Verify all required fields are present:
...
Write a complete, executable Python script to `{workspace}/{slug}/teardown.py`.
...
python3 {workspace}/{slug}/teardown.py
```

For dry-run first:
```bash
python3 {workspace}/{slug}/teardown.py --dry-run
```
...
Write `{workspace}/{slug}/{slug}-teardown-log.md`:
