...
**Machine-readable source:** `docs/ext-registry.yaml` — the authoritative registry consumed by `scripts/validate_ext_deps.py`.  
**This file:** human-readable detail (notes, API resource lists, decision rationale). Both files must stay in sync.

**Loaded by:** `skills/loom/SKILL.md` Step 0 (currency gate), and any skill that accesses an external repo.

**Path resolution shorthand:** In SKILL.md files, use `{hive-mind-root}`, `{workflows-root}`, `{agent-builder-sdk-root}`, etc. as placeholders. Each resolves to its `env_override` env var if set, or `local_path` from `docs/ext-registry.yaml` otherwise.

**Adding a repo:** Add one entry to `docs/ext-registry.yaml`, then add the detail section here. Run `scripts/validate_ext_deps.py` to confirm it resolves. Record the decision in `docs/decisions.md`.
