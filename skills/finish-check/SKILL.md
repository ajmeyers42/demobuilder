...
| C-7 | **D-026: Engagement tags** | Search `deploy/main.tf` and `deploy/bootstrap-data.py` for `loom:` tag string | Present in at least one resource in `main.tf` AND in `loom_tags()` function in `bootstrap-data.py` | WARN — re-run bolt-bootstrap |
