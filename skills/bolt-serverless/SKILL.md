---
name: bolt-serverless
description: >
  Serverless deployment variant for bolt-bootstrap. Generates Terraform HCL
...
- **ML field names:** Serverless uses `record_score`, `timestamp`, `partition_field_value`, `by_field_value` — NOT `anomaly_score`, `@timestamp`, `store_id`, `sku`. Confirmed by Step 1f in finish-verify.
...
`bolt-spin` created a Serverless project (otherwise omit the `ec` provider block).
...
(written by finish-verify Step 1f). Never use `anomaly_score`, `@timestamp`, `store_id`,
...
YAML/JSON (already handled by finish-verify using `asset-schema.json`), not in the
