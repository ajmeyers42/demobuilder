...
description: >
  ...
  running warp-listen or thread-audit. Run after thread-audit when
...
Scripts that skip any of these three elements are non-conformant and will fail `finish-check`.
...
`skills/warp-spark/SKILL.md` (which mirrors `hive-mind/skills/hive-sa-coaching/references/DEMO_ARCHETYPES.md`).
...
If no audit was run: "Platform audit not completed — run thread-audit before build."]
